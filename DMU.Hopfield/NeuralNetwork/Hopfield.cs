...
namespace DMU.NeuralNetwork
{
    public class Hopfield
    {
        ...
        /// </summary>
        /// <param name="trainData">Matrix containing the training pattern(s).</param>
        public Matrix Train(Matrix trainData)
        {
            //see if this is the first piece of training data
            if (_neuronCount == 0)
            {
                //get the size of the network, i.e. the number of neurons
                _neuronCount = trainData.RowCount;
            ...
            trainData = trainData.ToBiPolar();

            //transpose and multiply by self
            Matrix trainDataT = Matrix.Transpose(trainData);
            Matrix result = Matrix.Multiply(trainData, trainDataT);

            //neurons do not have conections to themselves so subtract a unit matrix (with the diagonal set
            //to the number of paterns)
            Matrix unit = new Matrix(trainData.RowCount, (double)trainData.ColumnCount);
            ...
            Matrix offsetNeuron = new Matrix(trainData.ColumnCount, 1, 1.0);

            //multiply offsetNeuron with input pattern(s) and add to _offsetNeuronWeights.
            result = Matrix.Multiply(trainData, offsetNeuron);
        ...
        {
            if (_neuronCount <= 0)
                throw new NetworkException(Resources.ErrorMessages.NetworkNotTrained);
            if (!testData.IsColumnVector)
                throw new ArgumentException(Resources.ErrorMessages.ArgumentMatrixisNotColumnVector);
            if (testData.RowCount != _neuronCount)
                throw new ArgumentException(Resources.ErrorMessages.ArgumentBadSize);
        ...
        /// <summary>
        /// KK Simple Fisher-Yates to shuffle an array.
        /// </summary>
